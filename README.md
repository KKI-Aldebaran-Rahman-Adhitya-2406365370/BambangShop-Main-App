...
1. In this implementation of the observer pattern, the push variation of the observer pattern is used. The main app, which is the publisher, actively pushes notifications to every subscriber by making HTTP POST requests to their respective URLs whenever an event occurs. The subscriber does not need to pull data from the publisher because the publisher is already pushing the data to the subscribers. 
2. If the pull variation was used instead, an advantage of it would be that the publisher does not need to know about the state of each subscriber's URL or state. This is because subscribers control when they receive data which reduces unnecessary notifications. However, a disadvantage would be that subscribers would need to continuously ask the publisher for updates which is inefficient especially for real-time notifications.
3. If multi-threading wasn't implemented, the notify() function would send HTTP requests to every subscriber sequentially. Any delay in one subscriber would significantly slow down the entire process and may halt the application. Multi-threading allows each notification to be sent concurrently which enables the app to stay responsive.